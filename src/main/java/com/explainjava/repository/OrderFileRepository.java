...
public class OrderFileRepository extends GenericRepository<Order> {
    ...
    public OrderFileRepository(String filename) throws IDNotUniqueException {
	super();
    ...
    @Override
    public Order save(Order order) throws IDNotUniqueException {
    ...
    public List<Order> readLemonadesFromFile() {
	...
	try {
		...
		String line = order.getId() + "," + order.getLemonade() + "," + order.getQuantity() + ","
			+ order.getFinalPrice() + "," + order.getDate();
		writer.write(line);
		writer.newLine();
	    }
	    writer.close();
